  ...
  if (user) {
    return (
      <div style={{display:'flex',alignItems:'center',gap:'.5rem',marginLeft:'.5rem'}}>
        <div style={{width:32,height:32,borderRadius:'50%',overflow:'hidden',background:'var(--sea)',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
          {profile?.avatar_url ? (
        ...
        <span style={{fontSize:'.85rem',fontWeight:500,color:'var(--fg)'}}>
          {profile?.full_name?.split(' ')[0] ?? 'Mon compte'}
        </span>
        <button
          onClick={signOut}
          style={{fontSize:'.8rem',color:'var(--muted)',background:'none',border:'none',cursor:'pointer',padding:'0',fontFamily:'inherit'}}
        >
          Déconnexion
        </button>
      </div>
