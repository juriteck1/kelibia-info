...
import { getImmoListings } from '@/lib/db'
import type { ImmoListing } from '@/types'
...
export default function ImmoPage() {
  ...
  const [listings, setListings] = useState<ImmoListing[]>([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    setLoading(true)
    getImmoListings().then(data => {
      setListings(data)
      setLoading(false)
    })
  }, [])

  const filtered = type === 'all' ? listings : listings.filter(i => i.type === type)

  const stats = {
    total:    listings.length,
    vente:    listings.filter(i => i.type === 'vente').length,
    location: listings.filter(i => i.type === 'location').length,
    vacances: listings.filter(i => i.type === 'vacances').length,
  }
    ...
    <div className="pt">
      ...
      <div className="container" style={{padding:'2rem 0 3rem'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'.85rem',marginBottom:'1.5rem'}}>
          {[
            [String(stats.total),    'Annonces actives'],
            [String(stats.vente),    'Ventes'],
            [String(stats.location), 'Locations'],
            [String(stats.vacances), 'Vacances'],
          ].map(([n,l]) => (
        ...
        {/* Chargement */}
        {loading ? (
          <div style={{textAlign:'center',padding:'3rem 0',color:'var(--muted)'}}>Chargement…</div>
        ) : (
